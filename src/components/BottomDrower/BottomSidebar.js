...
const BottomDrawer = ({
  ...
  const navLink = (name, icon, badge, indent = false, index, path) => (
    <li
      ...
      className={`flex items-center font-semibold ${
        name === "Games" ? "border-b-2 w-full border-[#2F4553] p-1.5" : ""
      }`}
    >
  ...
  const navItem = (item, index, indent = false) => {
    ...
    return (
      <div
        key={index}
        className="flex items-center p-2 px-0 hover:bg-[#2F4553] w-full "
      >
        {rest.to || rest.href ? (
          <Link
            {...(rest.to && { as: NavLink })}
            {...rest}
            onClick={() => handleItemClick(rest.to)}
          >
  ...
  return (
    <div
      className={`fixed left-0 right-0 bg-[#0f212e] shadow-lg p-4 transition-all duration-300 ease-in-out ${
        isOpen ? "bottom-[3.65rem] z-[1000] top-[3.5rem]" : ""
      } flex flex-col`}
    >
      ...
      <div className="py-4 flex justify-center space-x-5">
        <button
          ...
          className="text-white relative min-w-[5rem] min-h-[3rem] max-h-[5rem] flex justify-end bg-cover bg-[position:0_-15px] aspect-[3.5/1] group"
        >
          <img
            src={casino1}
            className={`h-full w-full rounded absolute transition-opacity duration-300 group-hover:opacity-100 ${
          ...
          />
          <span
            className={`absolute inset-0 flex justify-center items-center text-sm font-bold ${
              isOpen ? "block" : "hidden"
            }`}
          >
            CASINO
      ...
      <div className="overflow-y-auto flex-grow">
        ...
        {profilePopupOpen.isWalletOpen && (
          <Wallet
            closeWallet={() =>
              setProfilePopupOpen({ ...profilePopupOpen, isWalletOpen: false })
            }
          />
        )}
        {profilePopupOpen.isVaultOpen && (
          <Vault
            closeVault={() =>
              setProfilePopupOpen({ ...profilePopupOpen, isVaultOpen: false })
            }
          />
        )}
        {profilePopupOpen.isVipOpen && (
          <Vip
            closeVip={() =>
              setProfilePopupOpen({ ...profilePopupOpen, isVipOpen: false })
            }
          />
        )}
        {profilePopupOpen.isStatistic && (
          <Statistic
            closeStatistic={() =>
              setProfilePopupOpen({ ...profilePopupOpen, isStatistic: false })
            }
          />
        )}
        ...
        {profilePopupOpen.isNotification && (
          <Notification
            closeNotification={() =>
              setProfilePopupOpen({
                ...profilePopupOpen,
