...
function MyAllBet() {
  ...
  return (
    <div>
        ...
        <div className="flex justify-center mb-4">
          <div className="bg-[#1a2c38] flex rounded-full p-[4px] space-x-1 font-bold">
            {menuItems.map((item) => (
              <button
                key={item.label}
                className={`py-2 lg:px-5 md:px-5 rounded-full flex justify-center items-center text-sm ${
                  gameMenu === item.label
                    ? "bg-[#4d718768]"
                    : "hover:bg-[#4d718768]"
                }`}
      ...
      <div className="absolute bottom-[4.2rem] right-0 w-full ">
        <div className="bg-[#213743] p-4 space-y-2 ">
          <div className="text-center">
            <button className="text-white bg-[#2F4553] hover:bg-[#557086] text-sm p-2 w-full rounded font-semibold">
              View All
            </button>
          </div>
        </div>
