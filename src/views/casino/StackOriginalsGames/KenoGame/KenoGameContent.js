...
function KenoGameContent() {
  ...
  const renderButtons = () => {
    ...
    const xValueButton = (configArray) => {
      return configArray.map((value, index) => (
        index < length + 1 ? (
          <button key={index} className="text-white font-bold py-2 md:py-4 bg-[#2f4553] w-full rounded-md">
            {value}
          </button>
        ) : null
      ));
  ...
  return (
    <div className={`bg-[#0f212e] relative h-full flex flex-col items-center justify-center rounded-t-lg lg:p-4 md:p-1.5 ${isMobile ? ' max-sm:mx-2' : 'xl:w-[52rem] lg:w-[39rem]'}`}>
      <div className={`grid ${isMobile ? 'grid-cols-8 gap-1.5' : 'grid-cols-8 xl:gap-x-3 lg:gap-x-2 md:gap-x-3 gap-x-3 gap-y-1.5'} relative z-10 p-1.5 max-sm:w-full`}>
        {images?.map((img, index) => (
          <div
            key={index}
            className={`flex justify-center items-center w-full ${isMobile ? 'md:w-[2.6rem] p-2 md:h-[2.7rem] w-[2.5431rem] h-[2.5431rem] ' : 'xl:w-[5.3rem] lg:w-[4.2rem] xl:h-[5.3rem] lg:h-[4.2rem]'} bg-[#2f4553] rounded-lg hover:-translate-y-1 hover:bg-[#688a9f] ${zoomClass[index] ? "zoom-in-out" : ""}`}
      ...
      {selectTile.length > 0 ? (
        <div className="w-full pt-2">
          <div className="xl:px-4 lg:px-1 px-0 mb-3">
            {renderButtons()}
          </div>
          ...
          <div className="w-full xl:px-4 lg:px-1 px-0 mb-4 flex">
            {Array.from({ length: Math.min(selectTile.length + 1, 11) }).map((_, index) => (
              ...
              >
                {index}x
                <img src={kenoDaimond} className="xl:w-4 xl:h-4 lg:w-3 lg:h-3 md:w-2 md:h-2 w-2 h-2 ml-1" style={{ filter: "grayscale(100%)" }} alt="Not Found" />
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="w-full px-4 mb-4 lg:mt-14 md:mt-8 mt-10">
          <button className="text-[#d5dceb] text-sm md:py-2 xl:py-4 lg:py-3 py-2 bg-[#2f4553] font-semibold w-full rounded-md">Select 1 - 10 numbers to play</button>
        </div>
      )}
    </div>
  );
